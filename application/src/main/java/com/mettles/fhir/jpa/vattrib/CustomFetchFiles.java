...
public class CustomFetchFiles implements IFirstJobStepWorker<BulkImportJobParameters, NdJsonFileJson> {
	private static final Logger ourLog = LoggerFactory.getLogger(CustomFetchFiles.class);

	@Nonnull
	@Override
	public RunOutcome run(@Nonnull StepExecutionDetails<BulkImportJobParameters, VoidModel> theStepExecutionDetails,
		...
		try (CloseableHttpClient httpClient = newHttpClient(theStepExecutionDetails)) {
			...
			for (String nextUrl : urls) {
				...
				try (CloseableHttpResponse response = httpClient.execute(new HttpGet(nextUrl))) {
					...
					// Validate.isTrue(encoding == EncodingEnum.NDJSON, "Received non-NDJSON content
					// type \"%s\" from URL: %s", contentType, nextUrl);

					try (InputStream inputStream = response.getEntity().getContent()) {
						try (LineIterator lineIterator = new LineIterator(
								new InputStreamReader(inputStream, StandardCharsets.UTF_8))) {

							int chunkCount = 0;
							int lineCount = 0;
							StringBuilder builder = new StringBuilder();
							NdJsonFileJson singleResourceData = new NdJsonFileJson();
							...
							while (lineIterator.hasNext()) {
								...
								if (encoding == EncodingEnum.NDJSON) {
									System.out.println("processing nd json files");
									if (lineCount >= maxBatchResourceCount || charCount >= batchSizeChars
											|| !lineIterator.hasNext()) {

										ourLog.info("Loaded chunk {} of {} NDJSON file with {} resources from URL: {}",
												chunkCount, FileUtil.formatFileSize(charCount), lineCount, nextUrl);

										NdJsonFileJson data = new NdJsonFileJson();
										data.setNdJsonText(builder.toString());
										data.setSourceName("ndjson");
							...
							if ((encoding != EncodingEnum.NDJSON)&&(builder.length() > 0)) {
								singleResourceData.setNdJsonText(builder.toString());
								singleResourceData.setSourceName("json");
								theDataSink.accept(singleResourceData);
								builder.setLength(0);
							}
